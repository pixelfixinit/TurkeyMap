...
// @grant        none
// ==/UserScript==
var baseTemplateUrl = 'https://raw.githubusercontent.com/pixelfixinit/turkeymap/master/';
var vers = "TurkeyZone";
var range = 50
...
function startup() {
  ...
  div.innerHTML = '<style>.grecaptcha-badge,#message{display: none}.palette-footer{z-index:5}</style>\n' +
    '<div id="minimapbg" style="background-color:rgba(255,255,255,0.60); border-radius:0px; position:absolute; right:3px; bottom:3px; z-index:1;">' +
    ...
    '</div><div id="minimap-config" style="line-height:15px;padding:3px;background-color:rgba(255,255,255,0.75);color:rgb(255,0,0);">' +
    ' <span id="hide-map" style="cursor:pointer;">Sakla' +
    ' </span> | <span id="follow-mouse" style="cursor:pointer;">İmleci takip et' +
    '	</span> | <span id="toggle-grid" style="cursor:pointer;">Mangal' +
...
function toggleShow(newValue) {
  ...
  if (toggle_show) {
    ...
    minimap_text.innerHTML = "Minimapi göster";
    minimap_text.style.display = "block";
    minimap_text.style.color = "red";
...
function loadTemplates() {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    //    continue;
    if (!x_window.between(temp_x-range*1, temp_xr+range*1)) continue;
    if (!y_window.between(temp_y-range*1, temp_yb+range*1)) continue;
...
function loadImage(imagename) {
  ...
  if(src.indexOf("//") < 0) src = baseTemplateUrl + ("images/") + src;
